...
class _CartScreenState extends State<CartScreen> {
  ...
  // ===== نظام الحساب الدقيق =====
  // تحويل إلى عدد صحيح (3 خانات عشرية)
  ...
  // تحويل من عدد صحيح إلى عشري
  double _toDecimal(int intValue) => intValue / 1000;

  // تنسيق الرقم (3 خانات عشرية)
  String _format(double value) => value.toStringAsFixed(3);

  // حساب سعر العنصر × الكمية بدقة
  double _itemTotal(dynamic price, int quantity) {
    final priceInt = _toInt(price);
    return _toDecimal(priceInt * quantity);
  }
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      // استخدام الحساب الدقيق
      final items = _cart.map((item) {
        final price = _toDecimal(_toInt(item['price']));
        final qty = (item['quantity'] as int?) ?? 1;
        final itemTotal = _itemTotal(item['price'], qty);
        return {
          'name': item['name'],
          'price': price,
          'quantity': qty,
      ...
      final orderId = await FirebaseRestService.push('restaurant-system/restaurants/sham-coffee-1/orders', orderData);

      // Update table/room status if number provided
      if (locationNumber.isNotEmpty && orderId.isNotEmpty) {
        if (_locationType == 'table') {
          await FirebaseRestService.update(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${_format(_itemTotal(item['price'], item['quantity'] as int))} ر.ع',
                    style: const TextStyle(
                      fontSize: 14,
